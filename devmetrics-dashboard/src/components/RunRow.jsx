...
export default function RunRow({ run }) {
  ...
  const open = () => {
    navigate(`/sessions/${run.id}`);
  };

  return (
    <li
      role="row"
      aria-label={`${run.name || "Untitled run"}, ${
        active ? "live" : "ended"
      }`}
      tabIndex={0}
      onClick={open}
      ...
      className={cn(
        ...
        `md:grid md:${RUNS_GRID_COLS} md:items-center md:gap-4 md:px-6`
      )}
    >
      <div
        role="gridcell"
        className="flex items-start justify-between gap-3 md:contents"
      >
      ...
      <span
        role="gridcell"
        className="hidden justify-self-end font-data text-sm md:block"
      >
        {requests}
      </span>
      ...
      <span
        role="gridcell"
        className={cn(
          "hidden justify-self-end font-data text-sm md:block",
      ...
      <span
        role="gridcell"
        className="hidden justify-self-end font-data text-sm text-muted-foreground md:block"
      >
        {findings}
      </span>
