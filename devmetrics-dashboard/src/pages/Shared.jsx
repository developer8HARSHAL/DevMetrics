...
function findingTitle(finding) {
  const endpoint = finding.endpoint || "an endpoint";

  return (
    {
      error: `Errors on ${endpoint}`,
      error_burst: `Error burst on ${endpoint}`,
      retry_pattern: `Retry pattern on ${endpoint}`,
      duplicate_request: `Duplicate requests on ${endpoint}`,
      latency_anomaly: `Latency spike on ${endpoint}`,
    }[finding.type] || `Finding on ${endpoint}`
  );
}
...
function narrative(session, timeline, findings) {
  ...
  if (!findings.length) {
    return `${intro} No issues were detected.`;
  }

  if (critical.length) {
    return `${intro} ${findings.length} issue${
      findings.length === 1 ? "" : "s"
    } detected, ${critical.length} critical.`;
  }

  return `${intro} ${findings.length} issue${
    findings.length === 1 ? "" : "s"
  } detected, none critical.`;
}
    ...
    <div className="min-h-dvh bg-background">
      ...
      <article className="mx-auto max-w-3xl px-6 py-10">
        ...
        <p className="mt-4 text-body-sm leading-relaxed text-muted-foreground">
          {narrative(session, timeline, findings)}
        </p>
        ...
        {!findings.length && (
          <p className="mt-8 text-sm text-success">
            No issues were found in this run.
          </p>
        )}
              ...
              {findings.map((finding) => (
                ...
                >
                  ...
                  <p className="mt-1 font-data text-micro text-muted-foreground">
                    {Number(finding.occurrences) || 0} occurrence
                    {(Number(finding.occurrences) || 0) === 1
                      ? ""
                      : "s"}
                  </p>
                </div>
                ...
                <Card className="overflow-hidden p-0">
                  <div className="divide-y divide-border font-data text-micro">
                    {timeline.map((row) => (
                      <div
                        key={row.id}
                        className="flex gap-4 px-4 py-2 text-muted-foreground"
                      >
